        ...
        $.ajax({
            ...
            success: function(msg) {
                ...
                if (game.user.livestats) {
                    ...
                    setTimeout(
                        ajaxGetLiveStats,
                        game.databaseUpdateInterval * 1000
                    );
                }
            }
        });
    ...
    function Ghost(name, gridPosX, gridPosY, image, gridBaseX, gridBaseY) {
        ...
        this.draw = function(context) {
            if (this.dead) {
                ...
                if (
                    pacman.beastModeTimer < 50 &&
                    pacman.beastModeTimer % 8 > 1
                ) {
                    context.drawImage(
                        this.dazzleImg2,
                        this.posX,
        ...
        this.move = function() {
            ...
            if (this.ghostHouse == true) {
                ...
                if (this.name == "clyde") {
                    if (game.level < 4 || game.pillCount > 104 / 3)
                        this.stop = true;
                    else this.stop = false;
                }
                // Inky starts after 30 pills and only from the third level on
                if (this.name == "inky") {
                    if (game.level < 3 || game.pillCount > 104 - 30)
                        this.stop = true;
                    else this.stop = false;
                }
        ...
        this.getNextDirection = function() {
            ...
            if (this.dead) {
                ...
                switch (this.name) {
                    ...
                    case "inky":
                        var tX =
                            pacman.getGridPosX() + 2 * pacman.direction.dirX;
                        var tY =
                            pacman.getGridPosY() + 2 * pacman.direction.dirY;
                    ...
                    case "clyde":
                        ...
                        var tY = pacman.getGridPosY();
                        var dist = Math.sqrt(
                            Math.pow(pX - tX, 2) + Math.pow(pY - tY, 2)
                        );
        ...
        this.setRandomDirection = function() {
            ...
            switch (dir) {
                case 1:
                    if (this.getOppositeDirection().equals(up))
                        this.setDirection(down);
                    else this.setDirection(up);
                    break;
                case 2:
                    if (this.getOppositeDirection().equals(down))
                        this.setDirection(up);
