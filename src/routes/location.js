                    ...
                    getAzureCloudMetadata(function(err, c, z) {
                        if (err) {
                            ...
                            getGCPCloudMetadata(function(err, c, z) {
                                if (err) {
                                    // Try Openstack next
                                    getOpenStackCloudMetadata(function(
                                        err,
                                        c,
                                        z
                                    ) {
...
function getAzureCloudMetadata(callback) {
    ...
    var azureOptions = {
        ...
        port: 80,
        path:
            "/metadata/instance/compute/location?api-version=2017-04-02&format=text",
        method: "GET",
        timeout: 10000,
        headers: {
            Metadata: "true"
...
function getK8sCloudMetadata(callback) {
    ...
    try {
        var sa_token = fs.readFileSync(
            "/var/run/secrets/kubernetes.io/serviceaccount/token"
        );
        var ca_file = fs.readFileSync(
