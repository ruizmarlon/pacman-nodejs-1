        ...
        withEnv(["DOCKER_APPLICATION_FQDN=${DOCKER_IMAGE_REPOSITORY}.prod.${DOCKER_APPLICATION_DOMAIN}",
            ...
            if(DOCKER_ORCHESTRATOR.toLowerCase() == "kubernetes"){
                ...
                withEnv(["DOCKER_KUBE_CONTEXT=${DOCKER_KUBE_CONTEXT}", "DOCKER_KUBERNETES_NAMESPACE=${DOCKER_KUBERNETES_NAMESPACE}"]) {
                    sh 'envsubst < kubernetes/cluster/001_mongo_pvc.yml | kubectl --context=${DOCKER_KUBE_CONTEXT} --namespace=${DOCKER_KUBERNETES_NAMESPACE} apply -f -'
                    sh 'envsubst < kubernetes/cluster/002_mongo_deployment.yml | kubectl --context=${DOCKER_KUBE_CONTEXT} --namespace=${DOCKER_KUBERNETES_NAMESPACE} apply -f -'
                    sh 'envsubst < kubernetes/cluster/003_mongo_service.yml | kubectl --context=${DOCKER_KUBE_CONTEXT} --namespace=${DOCKER_KUBERNETES_NAMESPACE} apply -f -'
                    sh 'envsubst < kubernetes/cluster/004_pacman_deployment.yml | kubectl --context=${DOCKER_KUBE_CONTEXT} --namespace=${DOCKER_KUBERNETES_NAMESPACE} apply -f -'
                    sh 'envsubst < kubernetes/cluster/005_pacman_service.yml | kubectl --context=${DOCKER_KUBE_CONTEXT} --namespace=${DOCKER_KUBERNETES_NAMESPACE} apply -f -'
                    sh 'envsubst < kubernetes/cluster/006_pacman_ingress.yml | kubectl --context=${DOCKER_KUBE_CONTEXT} --namespace=${DOCKER_KUBERNETES_NAMESPACE} apply -f -'
